# ingest_medical_docs.py — PRODUCTION v6 (NON-BLOCKING, NO RPC DEPENDENCY)
...
CHECKPOINT_FILE = (BASE_DIR.parent / "data" / "processed" / "ingest_checkpoint_docs.json").resolve()
...
logger = logging.getLogger("menoeaze.ingest")
...
# ─────────────────────────────────────────────
# EMBEDDING (SAFE — NO TIMEOUT THREAD)
# ─────────────────────────────────────────────
...
def detect_source_type(path: Path) -> str:
    p = str(path).lower()
    if "clinical_books" in p:
        return "clinical_book"
    if "guideline" in p:
        return "guideline"
    return "pdf"
...
def run_ingestion():
    ...
    if not pdf_files:
        logger.error("[INGEST] No PDFs found in %s", PDF_DIR)
        return

    logger.info(f"[INGEST] Found {len(pdf_files)} PDFs in {PDF_DIR}")
    ...
    for pdf_path in tqdm(pdf_files, desc="Documents"):
    ...
    logger.info("=" * 50)
    logger.info("=== INGESTION PIPELINE REPORT ===")
    logger.info(f"Total Duration : {duration:.1f}s")
    logger.info(f"Docs Processed : {metrics['docs_processed']}")
    logger.info(f"Docs Skipped   : {metrics['docs_skipped']}")
    logger.info(f"Docs Failed    : {metrics['docs_failed']}")
